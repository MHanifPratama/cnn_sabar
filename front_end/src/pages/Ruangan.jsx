...
import DeleteRuanganModal from "../components/DeleteRuanganModal";
...
const Ruangan = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const response = await fetch("http://127.0.0.1:3001/api/v1/ruangan/", {
                    method: 'GET',
                    headers: {
    ...
    const ruanganEditHandler = async () => {
        if (!nama) {
            setShowFieldReqModal(true)
        } else {
            try {
                const token = sessionStorage.getItem('token');
                const response = await fetch(`http://127.0.0.1:3001/api/v1/ruangan/${selectedId}`, {
                    method: 'PATCH',
                    headers: {
                        "Content-Type": "application/json",
                        "Accept": "application/json",
            ...
            <div className="w-full h-screen overflow-x-auto">
                ...
                <Modal show={showAddModal} size="md" onClose={onCloseAddModal} popup>
                    ...
                    <div className="space-y-6">
                        <h3 className="text-xl text-center font-medium text-gray-900 dark:text-white">Add Ruangan</h3>
                        <div className="mb-2 block">
                            <Label htmlFor="nama" value="Ruangan" />
                        </div>
                        
                        <TextInput
                            id="nama" type="text" placeholder="Ruangan"
                            onChange={(event) => {
                ...
                <Modal show={showEditModal} size="md" onClose={onCloseEditModal} popup>
                    ...
                    <Modal.Body>
                        <div className="space-y-6">
                            <h3 className="text-xl text-center font-medium text-gray-900 dark:text-white">Edit Ruangan</h3>
                                <div className="mb-2 block">
                                    <Label htmlFor="nama" value="Ruangan" />
                                </div>
                ...
                <div className="flex max-w-md m-7 justify-between items-center relative">
                    <div className="mb-2 block flex-grow">
                        <Label className="text-xl" htmlFor="base" value="Search Ruangan" />
                ...
                <div className="m-auto ml-7 overflow-x-auto overflow-y-auto" style={{ height: "80%", width: "95%"}}>
                    <Table striped style={{ width: '100%', borderCollapse: 'collapse' }}>
                        <Table.Head className="sticky top-0 z-10 bg-cyan-600">
                            <Table.HeadCell className="bg-cyan-600 text-white" style={{ width: '80%' }}>Ruangan</Table.HeadCell>
                        ...
                        {userData && userData.filter((data) => {
                            return search.toLowerCase() === '' ? data : data.nama_ruangan.toLowerCase().includes(search);
                        }).map((data) => (
                ...
                <>
                    ...
                    <DeleteRuanganModal showDeleteModal={showDeleteModal} setShowDeleteModal={setShowDeleteModal} id={selectedId} setUserData={setUserData} userData={userData}/>
